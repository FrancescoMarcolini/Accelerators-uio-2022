...
%
N_part = 5000; % total particles to be tracked
...
for k=1:N_turns,
    ...
    % sextupole
    is_sextupole_present = 1;
    if( is_sextupole_present )
        ksext = 1000;
        lsext = 1;
        % add a sextupole at a given point in the ring
        x(2, n) = x(2, n) + 0.5*ksext*lsext*x(2, n).^2;
    end% if
end% for
...
for k=1:N_turns,
    ...
    plot(s(idx), x(1, idx)*1e3, ':r');
     ...
     figure(2);
     hold on;
     plot(x(1, idx(1))*1e3, x(2, idx(1))*1e3, 'or');
     pause;
end% for N_turns
