...
#  SLICE EMITTANCE, energy near the expected value
x=tfsObj.Data["X"][0:19999]
px=tfsObj.Data["PX"][0:19999]
pt=tfsObj.Data["PT"][0:19999]
min=0
max=5e-6
boolp=np.logical_and(np.abs(pt)>np.full(int(2e4),min), np.abs(pt)<np.full(int(2e4),max))
x=x[boolp]
px=px[boolp]

print(x.shape)
covx=np.cov(np.asarray([x,px]))
emittancex=np.sqrt(np.linalg.det(covx))

print('In the selected region{} <pt < {}:'.format(min,max))
print('\n emittance={:.4e}'. format(emittancex))
